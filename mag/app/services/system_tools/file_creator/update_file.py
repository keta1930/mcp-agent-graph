...
async def handler(user_id: str, **kwargs) -> Dict[str, Any]:
    """更新文件（字符串替换），支持从conversation或project更新"""
    try:
        ...
        if update_type == "content":
            # 根据scope选择正确的document_manager读取文件
            if scope == "project":
                current_content = await document_manager.read_file(
            ...
            # 根据scope选择正确的document_manager更新文件
            if scope == "project":
                minio_result = await document_manager.update_file(
                    user_id=user_id, project_id=resource_id, filename=filename, content=new_file_content
                )
            else:
                minio_result = await document_manager.update_file(
                    user_id=user_id, conversation_id=resource_id, filename=filename, content=new_file_content
                )
            ...
            success = await repository.update_file_metadata(
                resource_id=resource_id, filename=filename, size=minio_result["size"],
            ...
            success = await repository.update_file_metadata(
                resource_id=resource_id, filename=filename, summary=new_content, log_comment=log, agent=agent
            )
