            ...
            for input_node in node.get("input_nodes", []):
                if input_node == "start":
                    ...
                    if any(n["name"] == input_node for n in nodes):
                        input_is_decision = any(n["name"] == input_node and (len(n.get("output_nodes", [])) > 1 or n.get("handoffs") is not None) for n in nodes)
                        input_shape = "{" if input_is_decision else "["
                        input_end_shape = "}" if input_is_decision else "]"
                        connections.append(f"{input_node}{input_shape}\"{input_node}\"{input_end_shape} --> {node_name}{shape}\"{node_name}\"{end_shape}")
            ...
            for output_node in node.get("output_nodes", []):
                if output_node == "end":
                    ...
                    if any(n["name"] == output_node for n in nodes):
                        output_is_decision = any(n["name"] == output_node and (len(n.get("output_nodes", [])) > 1 or n.get("handoffs") is not None) for n in nodes)
                        output_shape = "{" if output_is_decision else "["
                        output_end_shape = "}" if output_is_decision else "]"
                        connections.append(f"{node_name}{shape}\"{node_name}\"{end_shape} --> {output_node}{output_shape}\"{output_node}\"{output_end_shape}")
